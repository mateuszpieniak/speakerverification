function llr = score_gmm_trials(modelFiles, testFiles, trials, ubmFilename)
...
if iscellstr(modelFiles),
    mlen = length(modelFiles);
    models = cell(mlen, 1);
    for ix = 1 : mlen,
        tmp = load(modelFiles{ix});
        models{ix} = tmp.gmm;
    end
elseif iscell(modelFiles),
    models = modelFiles;
else
    error('Oops! modelFiles should be a cell array!');
end
